...
import io
import sys
import superheros as superheroes
import math
import random
...
def create_hero(max_strength=100, weapons=False, armors=False, health=False):
    ...
    hero = [
        ...
        "San Francisco",
        "Bananas"]
    name = hero[random.randint(0, len(hero) - 1)]
    if health:
        power = health
    else:
...
def test_team_attack():
    ...
    assert team_two.members[0].current_health == 100

    team_one.attack(team_two)
    team_one.attack(team_two)
    team_one.attack(team_two)

    #assert team_two.members[0].current_health <= 0
...
def test_team_attack_kills():
    ...
    athena.add_armor(socks)
    team_two.add_hero(athena)
    assert team_one.members[0].kills == 0
    team_one.kill_team()
    assert team_one.members[0].kills == 1
